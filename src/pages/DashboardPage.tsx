...
const SURVEY_FLAG = "webridge_survey_eligible"; // 로컬스토리지 키
...
const DashboardPage = () => {
  ...
  // 대시보드 진입(또는 뒤로가기) 시, 이전에 "오류 확인"을 했으면 모달을 한 번 띄움
  useEffect(() => {
    if (localStorage.getItem(SURVEY_FLAG) === "1") {
      setOpenSurvey(true);
      localStorage.setItem(SURVEY_FLAG, "2"); // '보여줌' 상태로 전환(중복 방지)
    }
  }, []);
  ...
  // 설문 자격 플래그 소모(모달 닫기 시)
  const closeSurvey = () => {
    setOpenSurvey(false);
    // 설문을 이미 보여줬다면 플래그 제거/초기화
    localStorage.removeItem(SURVEY_FLAG);
  };

  // 스캔 시작 핸들러
  const handleStartScan = async () => {
    // ✅ 새로운 URL로 시작하려 할 때, 설문 자격이 있으면 먼저 모달부터
    const flag = localStorage.getItem(SURVEY_FLAG);
    if (flag === "1" || flag === "2") {
      setOpenSurvey(true);
      // flag는 유지(닫기 시 제거). 스캔은 모달 닫은 후 다시 시도하게 함.
      return;
    }
                        ...
                        <Button
                          ...
                          onClick={() => {
                            if (
                              ...
                              // "오류 확인" 버튼을 누르는 시점에 해당 항목에 이슈가 1개 이상이면 설문 자격 플래그 ON
                              if ((item as any).hasIssues) {
                                localStorage.setItem(SURVEY_FLAG, "1");
                              }
                              navigate(
